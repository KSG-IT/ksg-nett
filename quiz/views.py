...
def user_quiz_pool_helper(quiz,internal_group):
    ...
    if internal_group == "new-members":
        all_groups = InternalGroup.objects.filter(type=InternalGroup.Type.INTERNAL_GROUP).order_by('?')  
    else:
...
def quiz_detail_view(request, quiz_id):
    ...
    participant_current = Participant.objects.filter(quiz=quiz, correct_user__isnull=False, guessed_user__isnull=True).first()
    #current progress counter, we have a property which does the same job but with better performance
    quiz_progress = Participant.objects.filter(quiz=quiz, guessed_user__isnull=False).count()
...
def quiz_check(request, quiz_id, user_id):
    if request.method == 'POST':
        quiz = get_object_or_404(Quiz, pk=quiz_id)
        participant_guessed_on = Participant.objects.filter(quiz=quiz, correct_user__isnull=False, guessed_user__isnull=True).first()
        clicked_user = User.objects.filter(pk=user_id).first()
        participant_guessed_on.guessed_user=clicked_user
        print(f"Participant {participant_guessed_on.guessed_user} has been guessed, correct guess was {participant_guessed_on.correct_user}")
        participant_guessed_on.save()

        #------------------ This snippet creates a new participant to be guessed on
        users_available = quiz.fake_users.exclude(solution_in_quiz__quiz=quiz,solution_in_quiz__isnull=False)
        next_to_guess = choice(users_available)
