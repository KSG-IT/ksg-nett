...
class TestGetSemesterYearShortHand(TestCase):
    def test_get_semester_year_shorthand__timestamp_in_spring__returns_v_prefix_and_correct_year(
        self,
    ):
        timestamp = timezone.datetime(year=2018, month=3, day=1)
        self.assertEqual(get_semester_year_shorthand(timestamp), "V18")

    def test_get_semester_year_shorthand__timestamp_in_autumn__returns_h_prefix_and_correct_year(
        self,
    ):
...
class TestGetSemesterYearShortHandFilter(TestCase):
    ...
    def test_get_semester_year_shorthand_filter__timestamp_in_autumn__returns_h_prefix_and_correct_year(
        self,
    ):
        timestamp = timezone.datetime(year=2018, month=8, day=1)
        context = Context({"timestamp": timestamp})
...
class TestGetSemesterYearShorthandsByDate(TestCase):
    def test_get_semester_year_shorthands_by_date__argument_is_in_this_semester__returns_array_with_this_semester_only(
        self,
    ):
    ...
    def test_get_semester_year_shorthands_by_date__argument_is_5_semesters_back__returns_correct_result(
        self,
    ):
        with patch(
            "django.utils.timezone.now",
    ...
    def test_get_semester_year_shorthands_by_date__argument_is_in_the_future__returns_empty_list(
        self,
    ):
        with patch(
            "django.utils.timezone.now",
    ...
    def test_get_semester_year_shorthands_by_date__argument_is_around_century_change__returns_correct_result(
        self,
    ):
        with patch(
            "django.utils.timezone.now",
...
class TestGetSemesterYearShorthandsByCount(TestCase):
    def test_get_semester_year_shorthands_by_count__argument_is_one__returns_correct_result(
        self,
    ):
        # Patch timezone now, so we can make reliable tests without re-implementing the
        # method itself in the test
    ...
    def test_get_semester_year_shorthands_by_count__regular_positive_integer__returns_correct_result(
        self,
    ):
        with patch(
            "django.utils.timezone.now",
    ...
    def test_get_semester_year_shorthands_by_count__negative_integer__returns_empty_list(
        self,
    ):
        results = get_semester_year_shorthands_by_count(-1)
        self.assertListEqual(results, [])

    def test_get_semester_year_shorthands_by_count__timezone_now_around_century_change__returns_correct_result(
        self,
    ):
...
class TestGetSemesterDateBoundariesFromShorthand(TestCase):
    def test_get_semester_date_boundaries_from_shorthand__input_in_spring_and_2000s__returns_correct_result(
        self,
    ):
        start, end = get_semester_date_boundaries_from_shorthand("V18")
        expected_tzinfo = timezone.now().tzinfo
        self.assertEqual(start, timezone.datetime(2018, 1, 1, tzinfo=expected_tzinfo))
        self.assertEqual(end, timezone.datetime(2018, 7, 1, tzinfo=expected_tzinfo))

    def test_get_semester_date_boundaries_from_shorthand__input_in_autumn_and_2000s__returns_correct_result(
        self,
    ):
        start, end = get_semester_date_boundaries_from_shorthand("H18")
        expected_tzinfo = timezone.now().tzinfo
        self.assertEqual(start, timezone.datetime(2018, 7, 1, tzinfo=expected_tzinfo))
        self.assertEqual(end, timezone.datetime(2019, 1, 1, tzinfo=expected_tzinfo))

    def test_get_semester_date_boundaries_from_shorthand__input_in_spring_and_1900s__returns_correct_result(
        self,
    ):
        start, end = get_semester_date_boundaries_from_shorthand("V99")
        expected_tzinfo = timezone.now().tzinfo
        self.assertEqual(start, timezone.datetime(1999, 1, 1, tzinfo=expected_tzinfo))
        self.assertEqual(end, timezone.datetime(1999, 7, 1, tzinfo=expected_tzinfo))

    def test_get_semester_date_boundaries_from_shorthand__input_in_autumn_and_1900s__returns_correct_result(
        self,
    ):
        start, end = get_semester_date_boundaries_from_shorthand("H99")
        expected_tzinfo = timezone.now().tzinfo
        self.assertEqual(start, timezone.datetime(1999, 7, 1, tzinfo=expected_tzinfo))
        self.assertEqual(end, timezone.datetime(2000, 1, 1, tzinfo=expected_tzinfo))

    def test_get_semester_date_boundaries_from_shorthand__invalid_input__raises_valueerror(
        self,
    ):
        with self.assertRaises(ValueError):
