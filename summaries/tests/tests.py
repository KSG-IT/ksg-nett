...
class SummaryCreateTest(TestCase):
    ...
    def test_summary_create__POST_request_with_data__creates_new_summary(self):
        response = self.client.post(
            reverse(summaries_create),
            urlencode(
                {
                    "type": SummaryType.BARSJEF,
    ...
    def test_summary_create__POST_request_with_missing_data__renders_form_with_error(
        self,
    ):
        response = self.client.post(
            reverse(summaries_create), {"text": "This is a cool summary"}
        )
...
class SummaryUpdateTest(TestCase):
    ...
    def test_update_summary__POST_request_with_correct_data__updates_summary(self):
        response = self.client.post(
            reverse(summaries_update, kwargs={"summary_id": 1}),
            urlencode(
                {
    ...
    def test_update_summary__POST_request_with_missing_data__fails_with_error_in_form(
        self,
    ):
        response = self.client.post(
            reverse(summaries_update, kwargs={"summary_id": 1}),
